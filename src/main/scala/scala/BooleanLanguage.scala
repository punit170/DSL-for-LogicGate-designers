...
object BooleanLanguage:
  ...
  val GateTable: mutable.Map[String, LogicGates] = mutable.Map[String, LogicGates]()
  ...
  val EnvironmentTableMap: mutable.Map[String, mutable.Map[String, Boolean]] = mutable.Map[String, mutable.Map[String, Boolean]]()

  // to store transformer functions applied to MAP() and keep track of transformer function in the current scope 
  val monadicFuncStack: mutable.Stack[LogicGates => LogicGates] = mutable.Stack[LogicGates => LogicGates]()
  ...
  val logicGateStack, classStack, objectStack, interfaceStack, exceptionStack: mutable.Stack[String] = mutable.Stack[String]()
  ...
  case class tempClass(field: mutable.Map[String, Boolean|LogicGates], constructor: List[LogicGates], methods: mutable.Map[String, List[LogicGates]], ParentClass: Array[tempClass])

  // Environment Map to Store Classes
  val ClassMap: mutable.Map[String, tempClass] = mutable.Map[String, tempClass]()
  // Environment Map to Objects of defined classes
  val ObjectMap: mutable.Map[String, mutable.Map[String, tempClass]] = mutable.Map[String, mutable.Map[String, tempClass]]()
  ...
  val ObjectTable, InheritanceTable, ImplementsTable, ExceptionClassTable: mutable.Map[String, String] =  mutable.Map[String, String]()

  // Maps for access modifiers of classes: Array[0]: For List of Fields, Array[1]: For List of Methods
  val publicMap, privateMap, protectedMap: mutable.Map[String, Array[List[String]]] = mutable.Map[String, Array[List[String]]]()

  // Map to store overridden function names of a Class's Object
  val VirtualDispatchTable: mutable.Map[String, (String, ListBuffer[String])] =mutable.Map[String, (String, ListBuffer[String])]()

  // Map to store abstract methods of abstract classes
  val AbstractClassMap: mutable.Map[String, ListBuffer[String]] = mutable.Map[String, ListBuffer[String]]()

  // Sets to track abstract methods(to be implemented) and concrete methods for a concrete class
  val concreteMethodSet, abstractMethodSet, tempSet: mutable.Set[String] = mutable.Set[String]()

  // container to store interface as a combination of constant fields, abstract methods, and reference of a super interface
  case class tempInterface(field: mutable.Map[String, Boolean|LogicGates], methods: List[String], ParentInterface: Array[tempInterface])

  // Environment Map to store Interfaces
  val InterfaceMap: mutable.Map[String, tempInterface] = mutable.Map[String, tempInterface]()

  //Map to store ExceptionClass and its HandleException block. Map[ExceptionClassName -> Array[TryBlockExprs, CatchBlockExprs]]
  val ExceptionMap: mutable.Map[String, (List[LogicGates], List[LogicGates])] = mutable.Map[String, (List[LogicGates], List[LogicGates])]()
   
  
  
  //Defining data types and their evaluations for the language
  enum LogicGates extends Object:
    case Input(iName: String)
    case Value(v: Boolean)
    case LogicGate(logicName: String)
    case NOT(i1: LogicGates)
    case AND(i1: LogicGates, i2: LogicGates)
    case OR(i1: LogicGates, i2: LogicGates)
    case XOR(i1: LogicGates, i2: LogicGates)
    case NAND(i1: LogicGates, i2: LogicGates)
    case NOR(i1: LogicGates, i2: LogicGates)
    case XNOR(i1: LogicGates, i2: LogicGates)

    // To create logic gate variables or to create input variable bindings of some scope
    case Assign(varGate: LogicGate | Input | Field | ConstantField, gate: LogicGates)

    // To filter out current scope in order to assign input variables for respective logic gates
    case Scope(gate: LogicGate, exprs: List[LogicGates])
    ...
    case Method(mName: String, exprs: List[LogicGates], params: List[Assign])

    // Constructor of a Class- exprs: List of expressions the constructor takes
    case Constructor(exprs: List[LogicGates])
    ...
    case IF(condition: CheckEqual, thenClause: List[LogicGates], elseClause: List[LogicGates])
    ...
    case CheckEqual(i1: LogicGates, i2: LogicGates)

    // Function to evaluate data types
    def eval: Boolean | LogicGates = this match
      ...
      case AND(i1, i2) =>
        ...
        (res1, res2) match
          ...
          case (e1Bool: Boolean, e2: LogicGates) => AND(Value(e1Bool), e2)
          case (e1: LogicGates, e2Bool: Boolean) => AND(e1, Value(e2Bool))
          case (e1: LogicGates, e2: LogicGates) => AND(e1, e2)
      ...
      case OR(i1, i2) =>
        ...
        (res1, res2) match
          case (e1Bool: Boolean, e2Bool: Boolean) => e1Bool | e2Bool
          case (e1Bool: Boolean, e2: LogicGates) => OR(Value(e1Bool), e2)
          case (e1: LogicGates, e2Bool: Boolean) => OR(e1, Value(e2Bool))
          case (e1: LogicGates, e2: LogicGates) => OR(e1, e2)
      ...
      case XOR(i1, i2) =>
        ...
        (res1, res2) match
          case (e1Bool: Boolean, e2Bool: Boolean) => e1Bool ^ e2Bool
          case (e1Bool: Boolean, e2: LogicGates) => XOR(Value(e1Bool), e2)
          case (e1: LogicGates, e2Bool: Boolean) => XOR(e1, Value(e2Bool))
          case (e1: LogicGates, e2: LogicGates) => XOR(e1, e2)
      ...
      case Method(mName: String, exprs: List[LogicGates], paramList: List[Assign]) =>
      ...
      case ClassDef(className, fields, constructor, methods) =>
        ...
        //field Map
        val fieldMap = mutable.Map[String, Boolean|LogicGates]()
        for (field <- fields)
        ...
        //methods' Map
        val methodMap = mutable.Map[String, List[LogicGates]]()
        for (method <- methods)
      ...
      case InterfaceDef(interfaceName, _, assigns,abstractMethods) =>
        ...
        //ConstantFieldsMap
        val cFieldMap: mutable.Map[String, Boolean|LogicGates] = mutable.Map[String, Boolean|LogicGates]()
      ...
      case CheckEqual(i1, i2) =>
        ...
        (i1Res,i2Res) match
          ...
          case (r1Bool: Boolean,r2: LogicGates) => CheckEqual(Value(r1Bool),r2)
          case (r1: LogicGates,r2Bool: Boolean) => CheckEqual(r1,Value(r2Bool))
          case (r1: LogicGates,r2: LogicGates) => CheckEqual(r1,r2)
      ...
      this match
        ...
        case IF(condition, thenClause, elseClause) =>
          condition.MAP(f) match
            case c: CheckEqual =>
              val updatedThenClause, updatedElseClause: ListBuffer[LogicGates] = ListBuffer[LogicGates]()
              for (expr <- thenClause)
    ...
    def applyMapFunc(i: LogicGates): LogicGates =
    ...
    def getParentMethod(cName: String, mName: String): List[LogicGates] =
  ...
  def TestGate(gateName: String, checkVal: Boolean): Boolean =
    val gateEval = LogicGates.LogicGate(gateName).eval
    gateEval match
      case v: Boolean =>
        if v == checkVal then
          true
